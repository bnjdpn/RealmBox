  ...
  return (
    <main className="shell">
      ...
      {view === "welcome" && <section className="hero screen-enter">
        ...
        <div className="world-orb" aria-hidden="true"><div className="moon"/><div className="ridge ridge-back"/><div className="ridge ridge-front"/><div className="campfire">✦</div></div>
      </section>}
      ...
      {(view === "dashboard" || view === "running") && <section className="dashboard screen-enter">
        <aside className="character-panel"><div className="avatar" aria-hidden="true">B</div><p className="eyebrow">BON RETOUR</p><h2>{dashboard.playerName}</h2><p>{dashboard.className} · niveau {dashboard.level}</p><div className="world-status"><span className={dashboard.sessionRunning ? "status-live" : "status-ready"}/><div><strong>{dashboard.sessionRunning ? "Monde éveillé" : "Monde prêt"}</strong><small>{presetLabels[preset].title} · IA locale prête</small></div></div></aside>
