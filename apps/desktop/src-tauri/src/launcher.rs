...
impl<R: CommandRunner> LauncherService<R> {
    ...
    pub fn status(&self) -> LauncherStatus {
        match self.load_record() {
            Ok(Some(record))
                if record.openwow_executable.is_file() && record.compose_file.is_file() =>
            {
            ...
            Ok(Some(record)) => LauncherStatus {
                ...
                components: components(ComponentState::Error, record.bots_enabled),
            },
            ...
            Err(error) => LauncherStatus {
                ...
                account_name: None,
                account_password: None,
                components: components(ComponentState::Error, true),
            },
        }
    }
        ...
        F: FnMut(LauncherProgress),
    {
        ...
        let staging = self.app_data.join(".installing-v1");
        if staging.exists() {
        ...
        F: FnMut(LauncherProgress),
    {
        ...
        if let Some(process_id) = self.client_process_id.take() {
    ...
    fn load_record(&self) -> Result<Option<InstallationRecord>, String> {
        ...
        if record.schema_version != INSTALL_SCHEMA {
            return Err(format!(
                "version d’installation {} non prise en charge",
...
fn missing_status() -> LauncherStatus {
    LauncherStatus {
        ...
        account_name: None,
        account_password: None,
        components: components(ComponentState::Missing, true),
    }
}

fn components(state: ComponentState, bots_enabled: bool) -> Vec<LauncherComponent> {
    vec![
        LauncherComponent {
            id: "client",
            label: "Client de jeu",
...
mod tests {
    ...
    fn compose_command_order_keeps_runtime_details_typed() {
        ...
        assert_eq!(args[1], "-p");
        assert_eq!(args[2], "realmbox");
        assert_eq!(args.last(), Some(&OsString::from("database")));
        let runner = RecordingRunner::default();
        runner.run("docker", &args, None).expect("recorded");
        assert!(
            runner.commands.lock().expect("commands")[0].starts_with("docker compose -p realmbox")
        );
    }
}
