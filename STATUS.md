...
| Dépôt GitHub | public, socle de publication en place | `bnjdpn/RealmBox`, branche `main`, visibilité `PUBLIC` relue par API ; présentation français/anglais, contribution, sécurité, templates, Dependabot et CD de prerelease publiés ; la seconde validation commune a révélé que `cargo-deny` analysait des dépendances Linux hors cibles et refusait la licence AGPL du workspace : configuration bornée à macOS arm64/Windows x64 et contrôle local désormais réussi, nouvelle CI encore à relire |
